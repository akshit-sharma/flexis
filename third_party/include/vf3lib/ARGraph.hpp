...
namespace vflib
{
	...
	*/
	...
	public:
		...
		*/
		virtual Node GetNodeAttr(nodeID_t node) const = 0;
		/**
		...
		* @returns End node id.
		*/
		virtual nodeID_t GetOutEdge(nodeID_t node, uint32_t i, Edge *pattr) const = 0;

    virtual std::pair<nodeID_t, Edge> GetOutEdgeWeight(nodeID_t node, uint32_t i) const = 0;
	};
	/**
	* @class ARGraph
	...
	*/
	...
	public:
		ARGraph(const ARGLoader<Node, Edge> *loader);
		...
		Node GetNodeAttr(nodeID_t i) const;
    ...
    std::pair<nodeID_t, Edge> GetOutEdgeWeight(nodeID_t node, uint32_t i) const;
	...
	*/
	template <typename Node, typename Edge>
	inline Node ARGraph<Node, Edge>::GetNodeAttr(nodeID_t id) const
	{
		assert(id < n);
		return attr.at(id);
	}
	...
	*/
	...
	{
		assert(node < n);
		return in.at(node).size() + out.at(node).size();
	}
  ...
  template <typename Node, typename Edge>
	inline std::pair<nodeID_t, Edge> ARGraph<Node, Edge>::GetOutEdgeWeight(nodeID_t node, uint32_t i) const {
	  assert(node < n);
	  assert(i < out[node].size());
	  return std::make_pair(out[node][i], out_attr[node][i]);
	}
	/**
	...
	*/
	template <typename Node, typename Edge>
	ARGraph<Node, Edge>::ARGraph(const ARGLoader<Node, Edge> *loader)
	{
		n = loader->NodeCount();
                attr.resize(n);
                in.resize(n);
		...
		for (i = 0; i < n; i++)
		{
			...
			for (j = 0; j < k; j++)
			{
                                nodeID_t n2; // changed by me
                                std::tie(n2, out_attr[i][j]) = loader->GetOutEdgeWeight(i, j); // changed by me
				out[i][j]=n2;
